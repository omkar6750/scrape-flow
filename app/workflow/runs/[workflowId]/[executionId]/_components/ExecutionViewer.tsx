...
export default function ExecutionViewer({
	...
	const query = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(
				`/api/workflows/executions/${initialData!.id}`
			);
	...
	const phaseDetails = useQuery({
		...
		queryFn: async () => {
			const res = await fetch(`/api/workflows/phases/${selectedPhase}`);
			if (!res.ok) throw new Error("Failed to load phase details");
			return await res.json();
		},
	});
	...
	useEffect(() => {
		...
		if (isRunning) {
			const phasesToSelect = phases.toSorted(
			...
			<aside className="w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex flex-grow flex-col overflow-hidden">
						...
						value={
							<div className="font-semibold capitalize flex gap-2 items-center">
								<PhaseStatusBadge
									status={
										query.data
											?.status as ExecutionPhaseStatus
									}
								/>
				...
				<div className="overflow-auto h-full px-2 py-4 ">
					{query.data?.phases.map(
						(phase: ExecutionPhase, index: number) => (
							<Button
								key={phase.id}
								className="w-full justify-between p-0 [&_svg]:size-7  font-semibold"
								variant={
							...
							>
								...
								<PhaseStatusBadge
									status={
										phase.status as ExecutionPhaseStatus
									}
								/>
							</Button>
						)
